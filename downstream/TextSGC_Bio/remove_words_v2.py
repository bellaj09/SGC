...
def get_clean_words(docs):
    ...
    bar.finish()
    return clean_words
clean_words = get_clean_words(doc_content_list) # in the order of the entire corpus.txt, which should have same index/id as the paths document.
...
clean_corpus_str = '\n'.join(clean_docs) # each abstract, cleaned, stopwords removed, biolemmatised, tokenised by whitespace. in the order of the original corpus/dataset.txt, same index as data/dataset.txt
